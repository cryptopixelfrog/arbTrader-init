...
contract DyDxTxManager is DyDxFlashLoan{
  ...
  event swapToFrom(
    ...
    function directContractCall
    (
      bytes calldata _params
    )
      external
    {
      (bool _getToken, bytes memory script) = abi.decode(_params, (bool,bytes));
      ...
      discountCHI
  {
    ...
    bytes memory _data = abi.encode(_flashToken, _flashAmount, _balanceBefore, _tradeToken, _params);
      ...
      external onlyPool
  {
    (
      ...
      address tradeToken,
      bytes memory script
    ) = abi.decode(_data, (address, uint256, uint256, address, bytes));
    ...
    if (_reserve == address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)) {
        ...
        IERC20(_reserve).transfer(msg.sender, IERC20(_reserve).balanceOf(address(this)));
    } */
  ...
  function locationOf(bytes memory data, uint256 location) internal pure returns (uint256 result) {
      assembly {
